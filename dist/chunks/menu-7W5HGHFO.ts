...
function buildGoalMenuState(runtime) {
  ...
  const pausedByAutomaticLimit = goal?.status === "paused" && goal.safetyPauseCause === "continuation_limit";
  const waitingReason = goal?.waiting ? safeGoalMenuText(goal.waiting.reason) : void 0;
  const state = pausedByAutomaticLimit ? "Paused \u2014 automatic-work limit reached" : waitingReason ? `Waiting \u2014 ${waitingReason}` : displayStatus(goal?.status);
  ...
  const title = goal ? [
    ...
    `Usage: ${goal.tokenBudget === void 0 ? formatDuration(goal.timeUsedSeconds) : `${formatTokenCount2(goal.tokensUsed)}/${formatTokenCount2(goal.tokenBudget)}`}`,
    automaticResponses,
  ...
  const menu = defineMenu({
    ...
    screens: {
      ...
      "increase-budget": () => {
        ...
        return {
          ...
          lines: goal ? increaseTokenBudgetGuidance(goal, runtime.settings.continuationLimits.automaticTurns) : ["The budget-limited goal is no longer available. Return to the Goal menu."],
      ...
      status: () => ({
        kind: "detail",
        title: "Goal status",
        lines: runtime.activeGoal ? goalSummary(runtime.activeGoal, runtime.settings.continuationLimits.automaticTurns).split("\n") : ["No goal is currently set."],
        hint: "back"
      }),
      help: () => ({
        kind: "detail",
    ...
    actions: {
      ...
      "start-with-custom-budget": async ({ value, signal }) => {
        ...
        if (budget === void 0) {
          notifyTerminal(ctx.ui, "Enter a positive token amount, for example 25k, 300k, or 1.5m.", "warning");
      ...
      "submit-increase-budget": async ({ value, signal }) => {
        ...
        const budget = parseTokenBudget(value ?? "");
        if (budget === void 0) {
          notifyTerminal(ctx.ui, "Enter a positive token amount, for example 300k, 1.5m, or 300000.", "warning");
          return { kind: "rejected" };
        }
        if (!goal || !requireCurrentBudgetPreview(
          runtime,
          goal,
        ...
        if (budget <= goal.tokensUsed) {
          notifyTerminal(
            ctx.ui,
            `Enter a new cumulative total greater than current usage (${formatTokenCount(goal.tokensUsed)}).`,
            "warning"
          );
          return { kind: "rejected" };
        }
...
function increaseTokenBudgetGuidance(goal, automaticLimit) {
  return [
...
function suggestedIncreasedBudget(goal) {
...
function increaseBudgetPreview(goal, budget, automaticLimit) {
  return [
    `Goal: ${safeGoalMenuText(goal.text, 4e3)}`,
...
async function editFromMenu(runtime, commands, ctx) {
  ...
  if (goal.status === "active") {
    const confirmed = await ctx.ui.confirm(
      "Apply goal edit?",
      `Current goal:
${safeGoalMenuText(goal.text, 4e3)}
...
function requireCurrentMenuGoal(runtime, expected, ctx) {
  ...
  notifyTerminal(ctx.ui, "The active goal changed while the dialog was open. Reopen /goal and try again.", "warning");
  return false;
}
...
//# sourceMappingURL=menu-7W5HGHFO.ts.map
