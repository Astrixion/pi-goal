...
import {
  ...
  updateGoalUsage
} from "./chunk-RNWM7MOG.ts";

// src/command.ts
var MAX_OBJECTIVE_LENGTH = 4e3;
var REMOVED_QUEUE_COMMANDS = /* @__PURE__ */ new Set(["add", "prioritize", "drop-last", "skip", "push", "unshift", "pop", "shift"]);
...
function hasLeadingSummary(messages, boundary) {
...
function loadGoalStateFromSession(ctx) {
  ...
  const legacyEntry = entries.filter((entry) => entry.type === "custom" && entry.customType === LEGACY_GOALS_STATE_ENTRY_TYPE).pop();
  return legacyEntry ? loadLegacyGoalsState(legacyEntry.data) : emptyGoalState();
}
function loadCanonicalGoalState(data) {
  if (!isRecord2(data)) return emptyGoalState();
  ...
  let goal = rawGoal === null || !isCanonicalGoal(rawGoal) ? void 0 : normalizeLoadedGoal(rawGoal);
  if (goal?.status === "complete") goal = void 0;
  return { goal, legacyQueueState: void 0 };
}
...
function loadLegacyGoalsState(data) {
  ...
  const onlyGoal = rawGoals.length === 1 ? rawGoals[0] : void 0;
  if (onlyGoal && isCanonicalGoal(onlyGoal) && pendingAction === void 0) {
    return {
      goal: normalizeLoadedGoal(onlyGoal),
      legacyQueueState: void 0
    };
  }
  if (rawGoals.length === 0 && pendingAction === void 0) return emptyGoalState();
  return legacyQueueState({
...
function isStoredGoal(value) {
  if (!isRecord2(value)) return false;
  return typeof value.id === "string" && Boolean(value.id) && value.id === value.id.trim() && validObjective(value.text) && ["active", "queued", "paused", "blocked", "usage_limited", "budget_limited", "complete"].includes(
    String(value.status)
...
function legacyQueueState(legacyQueue) {
  return {
    goal: void 0,
...
function notifyWhenSessionAlive(ctx, message, level) {
  try {
...
//# sourceMappingURL=chunk-7FMBWODN.ts.map
